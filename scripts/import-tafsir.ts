...
async function main() {
  ...
  if (!filePath) {
    throw new Error("Usage: npm run content:import:tafsir -- data/sources/tafsir/source.json");
  }

  const result = await stageImportFromFile(filePath);
  console.log(`Staged tafsir import ${result.id} with ${result.totalRecords} rows.`);
}
