...
async function main() {
  ...
  if (!filePath) {
    throw new Error(
      "Usage: npm run content:import:translations -- data/sources/translations/source.json"
    );
  }

  const result = await stageImportFromFile(filePath);
  console.log(`Staged translation import ${result.id} with ${result.totalRecords} rows.`);
}
