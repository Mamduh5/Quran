...
- text
- footnotes optional; stored separately from translation text
- sourceId
- importId
- checksum
- active boolean default false
...
Source metadata that does not have dedicated columns, such as `downloadUrl`, `apiDocsUrl`, `downloadedAt`, original file checksums, processed file checksums, and source-specific keys, is validated in the source file or generated by the importer and stored in `ContentImport.manifestJson`. Source attribution, license, URL, version, approval status, and notes are also stored on `ContentSource`.

`Translation.footnotes` was added in migration `20260607000000_translation_footnotes` so QuranEnc footnotes can be displayed separately without appending them to Quran text or translation text.
